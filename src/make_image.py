...
class Lungs(Dataset):
    ...
    def __getitem__(self, i):
        try:
            ...
            with open(Path(SAVEDIR) / 'errors.txt', 'a', encoding='utf-8') as f:
                f.write(f"{dset.dicom_folders[i]}\n")
                f.write(f"{e}\n")
            return None


SAVEDIR = 'data/train-jpegs-512'
MAX_LENGTH = 512.

if not osp.exists(SAVEDIR): os.makedirs(SAVEDIR)

df = pd.read_csv('data/train.csv')
dicom_folders = list(('data/train/' 
    + df.StudyInstanceUID + '/'+ df.SeriesInstanceUID).unique())
