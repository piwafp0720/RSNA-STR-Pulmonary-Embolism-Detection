...
def load_pretrained(model,
    ...
    if in_chans == 1:
        ...
        _logger.info('Converting first conv (%s) from 3 to 1 channel' %
                     conv1_name)
        conv1_weight = state_dict[conv1_name + '.weight']
        state_dict[conv1_name + '.weight'] = conv1_weight.sum(dim=1,
                                                              keepdim=True)
    elif in_chans != 3:
...
def adapt_model_from_string(parent_module, model_string):
    ...
    for n, m in parent_module.named_modules():
        ...
        if isinstance(old_module, nn.Linear):
            new_fc = nn.Linear(in_features=state_dict[n + '.weight'][1],
