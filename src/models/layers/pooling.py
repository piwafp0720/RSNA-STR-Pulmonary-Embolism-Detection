...
from functions.pooling import gem, mac, rmac, roipool, spoc
...
class Rpool(nn.Module):
    ...
    def forward(self, x, aggregate=True):
        # features -> roipool
        o = roipool(x, self.rpool, self.L,
                    self.eps)  # size: #im, #reg, D, 1, 1
    ...
    def __repr__(self):
        return super(Rpool, self).__repr__() + '(' + 'L=' + '{}'.format(
            self.L) + ')'
