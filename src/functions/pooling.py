...
def rmac(x, L=3, eps=1e-6):
    ...
    steps = torch.Tensor([2, 3, 4, 5, 6,
                          7])  # possible regions for the long dimension
    ...
    b = (max(H, W) - w) / (steps - 1)
    (tmp, idx) = torch.min(torch.abs(((w**2 - w * b) / w**2) - ovr),
                           0)  # steps(idx) regions for long dimension
    ...
    for l in range(1, L + 1):
        ...
        for i_ in cenH.tolist():
            for j_ in cenW.tolist():
                ...
                vt = vt / (torch.norm(vt, p=2, dim=1, keepdim=True) +
                           eps).expand_as(vt)
                v += vt
...
def roipool(x, rpool, L=3, eps=1e-6):
    ...
    b = (max(H, W) - w) / (steps - 1)
    _, idx = torch.min(torch.abs(((w**2 - w * b) / w**2) - ovr),
                       0)  # steps(idx) regions for long dimension
